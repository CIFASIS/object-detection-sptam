...
#include "../../../sptam/StereoProcessing/StereoProcessing.h"
#include "../../../sptam/Localization/Tracker.h"
...
int main(int argc, char *argv[])
{
  ...
  double keyFrameDistance = ptamParameters->GetKeyFrameDistance();
  double framesBetweenKeyFrames = ptamParameters->GetFramesBetweenKeyFrames();
  double epipolarDistance = ptamParameters->GetMatchingEpipolarDistance();
  double matchingDistance = ptamParameters->GetMatchingDistance();
  double matchingNeighborhood = ptamParameters->GetMatchingNeighborhood();
...
//  std::cout << "translationRight: " << cameraCalibrationRight.GetTranslation() << std::endl;
  ...
  Map map;
  MapMaker mapMaker(cameraCalibrationLeft,
                    cameraCalibrationRight,
                    featureDetector,
                    descriptorExtractor,
                    descriptorMatcher,
  ...
  // Fill MapMaker with PTAM parameters
  mapMaker.SetEpipolarDistance(epipolarDistance);
  mapMaker.SetKeyFrameDistance(keyFrameDistance);
  mapMaker.SetMatchingDistance(matchingDistance);
  mapMaker.SetMatchingNeighborhood(matchingNeighborhood);
  mapMaker.SetFrustumNearPlaneDist(nearPlaneDist);
  mapMaker.SetFrustumFarPlaneDist(farPlaneDist);
...
//  pointCloud.Start();
  ...
  // Main loop
  while(hasNextFrameLeft && hasNextFrameRight && !programMustEnd && !mapInitialized) {
...
//    }
     ...
     mapInitialized = mapMaker.InitFromStereo(frame,
                             cameraCalibrationLeft.GetIntrinsic(),
                             cameraCalibrationRight.GetIntrinsic(),
                             cameraCalibrationLeft.GetCameraPose(),
                             cameraCalibrationRight.GetCameraPose());
