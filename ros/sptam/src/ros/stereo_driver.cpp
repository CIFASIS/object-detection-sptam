 ...
 *
 * Copyright (C) 2015 Taihú Pire and Thomas Fischer
 ...
 *
 * Authors:  Taihú Pire <tpire at dc dot uba dot ar>
...
void sptam::stereo_driver::onDetection(const dl_node::DetectionWithPoseListConstPtr& detections_left)
{
  ...
  CameraPose kf_delta = sptam_->cp_list_[detections_left->kf_id] ;
  ...
  for (int i = 0;i<detsize;i++) {
    ...
    ObjectMeasurement detection = ObjectMeasurement(detections_left->data[i].cls,0.0,det_kf,kf_delta,
 ...
 *  *
     */
     ...
     double_t iou_val ;
     sptam::ObjectMap::SharedObject iou_candidate = sptam_->omap.MaxIOUCandidate(sDetection,iou_val) ;
     ...
     std::cout << "borrar " << iou_val << std::endl ;
    ...
    Eigen::Vector3d up_kf = -kf_orientation.col(1) ;
    Eigen::Vector3d up_world(0.0,-1.0,0.0) ;
    ...
    Eigen::Vector3d T = sDetection->GetObjPose(detections_left->data[i].yaw,up_kf,objDim,R) ;
    ...
    //Camera bbox_cam = sDetection->GetBBoxCamera(up_kf,*cameraParametersLeft_) ;
    ...
    /* Lock for reading a small segment of the Map, after getting the required points locking is no longer
       ...
       for (const auto& measurement : det_kf->measurements() ) {
          ...
          cv::Point2d projection = project( kf_cam.GetProjection(), eigen2cv( point ) );
    ...
    cv::Matx44d transRt = computeTransformation44(kf_cam.GetPosition(),kf_cam.GetPose().GetOrientationMatrix()) ;
    ...
    double_t obj_depth = depthFromMapPoints(kf_cam.GetProjection(),transRt,filtered_points,detections_left->data[i].x1+detections_left->data[i].x2,detections_left->data[i].y1+detections_left->data[i].y2,depthPoint) ;  
    ...
    Eigen::Vector3d Tdepth(T(0),T(1),obj_depth) ;
    
    Tdepth = Tdepth + faceCorrection ;  
  
    Eigen::Matrix3d WF_O = kf_orientation * R ;
    Eigen::Vector3d WF_P = kf_orientation * Tdepth + kf_position ;


    Eigen::Vector3d WF_normal = sDetection->ObjectNormalWorld(objAngle,up_world) ;

    // Fusion	 

    ObjectEdge edge(det_kf,Tdepth,R,objDim,WF_normal) ;
    SharedObjectEdge sEdge(new ObjectEdge(edge)) ;
    sObject->AddEdge(sEdge) ;   

 
    sObject->updateEstimateMedian(up_world) ;
  ...
  sptam_->omap.AddOutliers( kf_cam) ;
