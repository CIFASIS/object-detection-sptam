...
MapMakerThread::MapMakerThread(sptam::Map& map, const Parameters& params)
  : MapMaker( map, params ), stop_( false )
{
...
void MapMakerThread::attendNewKeyframes()
{
  ...
  std::vector< std::pair<sptam::Map::SharedKeyFrame, size_t> > covisibilityRecentKeyframes = recentKeyframe->covisibilityKeyFramesVector();
  ...
  // when the last keyframe came in)
  {
    ...
    /*int nFound = */ReFindNewlyMade( ListIterable<sptam::Map::SharedKeyFrame>::from( LBA_keyframes_window_ ), ListIterable<sptam::Map::SharedPoint>::from( new_points ) );
