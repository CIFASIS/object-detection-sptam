                      ...
                      size_t& nMatches, size_t& inliers, cv::Matx44d& estimated_pose)
{
  ...
  /* TODO: Gaston: Esto esta MAAAL al generar estos vectores de keypoints "desfasados" con respecto a los contenidos en los "ImageFeatures"
           estamos rompiendo la consistencia de los DMatch que podriamos crear despues! */
  cv::Mat descriptors1;
  cv::Mat descriptors2;
...
void LoopClosing::maintenance()
{
  ...
  while ( not stop_ ) {
    ...
    const sptam::Map::SharedKeyFrameSet& safe_window = mapper_.lockUsageWindow();
    ...
    #ifdef SHOW_PROFILING
      t_mapping_pause.stop();
      WriteToLog(" lc safe_map_update: ", t_mapping_pause);
    #endif
