...
import rospy
from dl_node.msg import Detection
from dl_node.msg import DetectionList
...
prototxt = os.path.join(cfg.MODELS_DIR, 'modelnet', NETS[demo_net][0],
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        for i in inds:
           ...
           if show == 1 :
              ...
              cv2.rectangle(im, (x1, y2 - ret[1] - baseline),
                            (x1 + ret[0], y2), (0, 0, 255), -1)
              cv2.putText(im, CLASSES[cls_ind]+str(dets[i,-1]), (x1, y2 - baseline),
              cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 1, CV_AA)
