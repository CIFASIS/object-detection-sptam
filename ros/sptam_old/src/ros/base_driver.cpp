  ...
  , transform_listener_( tfBuffer_ )
{
  ...
  //branch
  pub_kf_ = nhp.advertise<sptam::StereoKFwithPose>("/sptam/keyframe",1); 
  kf_id_ = 0 ;
  //
...
void sptam::base_driver::onFrame(const uint32_t seq, const ros::Time& time, StereoFrame& frame, TrackingReport& tracking_report)
{
  ...
  else
  {
    ...
    //branch
    if ((sptam_->frames_since_last_kf() % 15)  >= 0) {
       //Codigo viejo 
       //sptam_->kf_list_.push_back(sptam_->lastKeyFrame()) ;
       //Codigo nuevo
       sptam_->kf_list_.push_back(sptam_->referenceKeyFrame()) ;
       
       CameraPose kf_pose = sptam_->referenceKeyFrame()->GetCameraPose() ;
       
       Eigen::Matrix3d kf_OT = kf_pose.GetOrientationMatrix().transpose();
       Eigen::Matrix3d delta_O = kf_OT * refined_camera_pose.GetOrientationMatrix() ;
       Eigen::Vector3d delta_P = kf_OT * refined_camera_pose.GetPosition() - kf_OT * kf_pose.GetPosition();
       
       //Transformation from refined pose to reference keyframe
       CameraPose delta_kf(delta_P,Eigen::Quaterniond(delta_O),Eigen::Matrix6d::Identity()) ;
       //Identity transformation for test
       //CameraPose delta_kf(Eigen::Vector3d(.0,.0,.0),Eigen::Quaterniond::Identity(),Eigen::Matrix6d::Identity()) ;
       
       sptam_->delta_kf_list_.push_back(delta_kf) ;
