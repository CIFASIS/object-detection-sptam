  ...
  public:
    ...
    inline void Print() {
      ...
      for (const auto& object : list_) {
          Eigen::Vector3d pos = object->GetPosition() ;
          Eigen::Vector3d dim = object->GetDimensions() ;
          Eigen::Vector3d normal = object->GetNormal() ;
          double_t angle_degrees = atan2(-normal(0),normal(2)) ; //respectoal eje z de la camara
          if (object->IsGood()) std::cout << "*" ;
          std::cout << "Object " << object->GetClassName() << " Inlier: " << object->InlierCount() << " Outlier: " << object->OutlierCount() << std::endl ;
          std::cout << "|P: " << pos(0) << ' ' <<  pos(1) << ' ' << pos(2)
                   << "|D: " << dim(0) << ' ' <<  dim(1) << ' ' << dim(2) 
                   << "|a: " << angle_degrees*180/3.1416 << std::endl;
      }
      std::cout << "- - - - - - -" << std::endl ;
      for (const auto& object : deleted_) {
          Eigen::Vector3d pos = object->GetPosition() ;
          Eigen::Vector3d dim = object->GetDimensions() ;
          Eigen::Vector3d normal = object->GetNormal() ;
          double_t angle_degrees = atan2(-normal(0),normal(2)) ; //respectoal eje z de la camara
    ...
    */
    ...
    void MergeObjects(const SharedObject& a,const SharedObject& b) ;

    void ReviseMerge(const SharedObject& obj, const ObjectList& candidates) ;    

 
    SharedObject MaxIOUCandidate(const SharedObjectMeasurement detection, double_t& iou_val,ObjectList& high_iou) ;
