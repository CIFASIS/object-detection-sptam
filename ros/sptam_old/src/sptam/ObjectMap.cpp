...
namespace sptam
{
    ...
    ObjectMap::SharedObject ObjectMap::MaxIOUCandidate(const SharedObjectMeasurement detection,double_t& iou_val, ObjectList& iou_high) {
        
        double_t max_iou = 0.0 ;
        const double_t high_iou_thresh = 0.6 ;
        ...
        for (const auto& candidate : list_) {
            ...
            if (detection->GetSrcCamera().CanView(candidate->GetPosition())) {
                ...
                if (iou > high_iou_thresh) {
                  iou_high.push_back(candidate) ; 
                }
   ...
   { 
      const double_t MAX_OUTLIER_COUNT_DISTANCE = 4.0 ;
      for (const auto& object : list_) {
          if (src_cam.CanView(object->GetPosition())) {
            if (object->GetHit()) {
              ...
              double_t distance = (object->GetPosition() - src_cam.GetPosition()).norm() ;
              Eigen::Vector4d bbox = this->BBoxPrediction(src_cam,object) ;
              if ( (distance < MAX_OUTLIER_COUNT_DISTANCE) &&
                   (size_t(bbox(0)) > 0) &&
                   (size_t(bbox(1)) > 0) &&
                   (size_t(bbox(2)) < src_cam.GetCalibration().imageWidth()) && 
                   (size_t(bbox(3)) < src_cam.GetCalibration().imageHeight()) ) 
                   object->IncreaseOutlierCount() ;
            }
            if (object->IsDeletable() && object->IsGood()) deleted_.push_back(object) ;  
          }
       }
   ...
   void ObjectMap::MergeObjects(const SharedObject& a,const SharedObject& b) 
   {
     /*Add MapPoints*/
     a->AddMapPoints(b->GetMapPoints())  ;
     /*Add Measurements*/
     a->AddMeasurements(b->GetMeasurements()) ; 
     /*Add ObjectEdges*/
     a->AddEdges(b->GetEdges()) ;
     /* keep lowest Id*/
     size_t id_a = a->GetId() ;
     size_t id_b = b->GetId() ;
     if (id_a > id_b) a->SetId(id_b) ;
     
     /*Add Inlier and Outlier stats*/
     a->IncreaseOutlierCount(b->OutlierCount()) ;
     a->IncreaseInlierCount(b->InlierCount()) ;

     /*nothing left to do (?)*/
     return ;
   }

   void ObjectMap::ReviseMerge(const SharedObject& obj, const ObjectList& list) 
   {
     /*similarity threshold in meters*/
     const double_t similarity_thresh = 0.33 * obj->GetDimensions()(1) ;  
     size_t obj_id = obj->GetId() ;    

     for (const auto& candidate : list) {
        if (obj_id == candidate->GetId())
          continue ; //skip merging object with itself 
        if ((obj->GetClass() == candidate->GetClass()) &&
            ((obj->GetPosition() - candidate->GetPosition()).norm() < similarity_thresh)) 
        {
          std::cout << "@^@ Merged Objects " << obj->GetClassName() << " <- " << candidate->GetClassName() << std::endl ; 
          MergeObjects(obj,candidate) ;
          candidate->OkToDelete() ; 
        }
     }
